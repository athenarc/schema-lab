...
const CreateExperiment = () => {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      setErrorMessage(
        <>
          Experiment name <strong>{experimentName}</strong> must be unique.
  ...
  return (
    <Container className="py-5">
      ...
      <Card className="border-0 shadow-sm rounded-3 mb-4">
        <Card.Body>
          ...
          <Form onSubmit={handleSubmit}>
            <Card className="border-0 shadow-sm rounded-3 mb-4">
              <Card.Header
                className={`bg-primary text-white ${activeKey === "0" ? "border-bottom" : ""}`}
              >
                Basic Information
              </Card.Header>
              <Card.Body>
                  ...
                  <tbody>
                    {selectedTasks.length === 0 ? (
                      ...
                      selectedTasks.map((task) => (
                        <tr key={task.uuid}>
                          <td>
                            <Link to={`/task-details/${task.uuid}/executors`}>
                              {task.uuid}
                            </Link>
                          </td>
                          <td>
                            <TaskStatus status={task.current_status.status} />
                          </td>
                          <td>
                            {new Date(task.submitted_at).toLocaleString("en")}
                          </td>
                          <td>
                            {new Date(
                              task.current_status.updated_at,
                            ).toLocaleString("en")}
                          </td>
                        </tr>
                      ))
      ...
      <Modal show={showConfirmModal} onHide={handleCancelBack}>
        <Modal.Body>Are you sure you want to save this experiment?</Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>Your experiment has been created successfully.</Modal.Body>
        <Modal.Footer>
          <Button
            variant="primary"
            onClick={() => {
