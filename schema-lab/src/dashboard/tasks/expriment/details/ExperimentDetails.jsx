...
const ExperimentDetails = () => {
  ...
  useEffect(() => {
    const fetchExperimentDetails = async () => {
      try {
        ...
        if (!response.ok)
          throw new Error(
            `Error fetching experiment details. Status: ${response.status}`,
          );
    ...
    const fetchWorkflowTasks = async () => {
      try {
        const response = await getExperimentWorkflowDetails({
          creator,
          name,
          auth: userDetails.apiKey,
        });
        if (!response.ok)
          throw new Error(
            `Error fetching workflow tasks. Status: ${response.status}`,
          );
        const data = await response.json();
  ...
  const renderTaskRow = (task) => (
    <tr key={task.uuid}>
      <td>
        <Link
          to={`/task-details/${task.uuid}/executors`}
          state={{
            from: "experiments",
            creator: experimentDetails.creator,
