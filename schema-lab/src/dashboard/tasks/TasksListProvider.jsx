...
import { listStreamingTasks } from "../../api/v1/actions";
...
export const useTaskData = (
  ...
  useEffect(() => {
    ...
    if (showTasks !== isTasksView) {
      fetchTaskData(showWorkflowTasks, showStreamingTasks, showTasks, true); // Reset page when switching to tasks view
    }
    if (showWorkflowTasks !== isWorkflowView) {
      fetchTaskData(showWorkflowTasks, showStreamingTasks, showTasks, true); // Reset page when switching to workflow view
    }
    if (showStreamingTasks !== isStreamingView) {
      fetchTaskData(showWorkflowTasks, showStreamingTasks, showTasks, true); // Force fetch for streaming tasks
    }
  }, [
    showWorkflowTasks,
    isWorkflowView,
...
const TasksListProvider = ({ children, initialFilters = {} }) => {
  ...
  const [isTasksView, setIsTasksView] = useState(true);
  ...
  const fetchTaskData = useCallback(
    (
      isWorkflowTasks = isWorkflowView,
      isStreamingTasks = isStreamingView,
      isTasks = isTasksView,
      resetPage = false,
    ) => {
      setIsLoading(true);
      ...
      if (resetPage && isStreamingTasks !== isStreamingView) {
        filtersToUse = { ...filtersToUse, page: 0 };
        setTaskFilters(filtersToUse);
      }
      ...
      setIsStreamingView(isStreamingTasks);
      setIsTasksView(isTasks);

      const requestFilters = {
        ...filtersToUse,
        statuses: Object.keys(filtersToUse.statuses).filter(
          (k) => filtersToUse.statuses[k],
        ),
        view: "detailed",
        limit: pageSize,
