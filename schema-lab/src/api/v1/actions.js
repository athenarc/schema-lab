...
export const listTasks = (options = {}) => {
  ...
  if (filters.statuses)
    queryParameters.push(
      ...filters.statuses.map((s) => `status=${s.toUpperCase()}`),
    );
...
export const getExperiments = (options = {}) => {
  ...
  const queryString = queryParameters.length
    ...
    : "";
  return apiFetch(
    `${config.api.url}/reproducibility/experiments${queryString}`,
    {
      method: "GET",
      headers,
    },
    ...
    .then((data) =>
      Array.isArray(data) ? { count: data.length, results: data } : data,
    );
};

export const getExperimentDetails = ({ creator, name, auth }) => {
  return apiFetch(
    `${config.api.url}/reproducibility/experiments/${creator}/${name}`,
    {
      method: "GET",
      headers: { Authorization: `Bearer ${auth}` },
    },
...
export const putExperimentTasks = (apiKey, creator, name, uuid) => {
  return apiFetch(
    `${config.api.url}/reproducibility/experiments/${creator}/${name}/tasks`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
export const getExperimentWorkflowDetails = ({ creator, name, auth }) => {
  return apiFetch(
    `${config.api.url}/reproducibility/experiments/${creator}/${name}/workflows`,
    {
      method: "GET",
      headers: { Authorization: `Bearer ${auth}` },
    },
...
export const listStreamingTasks = (options = {}) => {
  const headers = options.auth
    ? { Authorization: `Bearer ${options.auth}` }
    : {};
  const filters = options.filters || {};
...
export const runStreamingTaskPost = (apiKey, requestData) => {
  return apiFetch(`${config.api.url}/api/streaming`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${apiKey}`,
...
export const cancelStreamingTask = ({ taskUUID, auth }) => {
  ...
  const queryString = queryParameters.length
    ...
    : "";
  return apiFetch(
    `${config.api.url}/api/streaming/${taskUUID}/terminate${queryString}`,
    {
      method: "POST",
      headers: { Authorization: `Bearer ${auth}` },
    },
...
export const retrieveStreamingTaskDetails = ({ taskUUID, auth }) => {
  const queryParameters = [];
  queryParameters.push(`streaming=leaf-influx`);
  const queryString = queryParameters.length
    ? `?${queryParameters.join("&")}`
    : "";
  return apiFetch(`${config.api.url}/api/streaming/${taskUUID}${queryString}`, {
    method: "GET",
    headers: { Authorization: `Bearer ${auth}` },
  });
};
